...
1. Data model (`US-01`)
2. Draft creation and load APIs (`US-02`, `US-03`)
3. Core cell editing and change tracking (`US-06`, `US-07`)
4. Structural matrix editing APIs for stages and lenses (`US-04`, `US-05`)
5. Conversation persistence and AI orchestration (`US-08`, `US-09`, `US-10`)
6. Frontend data integration (`US-11`, `US-12`, `US-13`, `US-14`)
7. Validation and smoke testing (`US-15`, `US-16`)
...
- **Acceptance Criteria:**
  ...
  - Conversation records and/or messages preserve enough mode context to distinguish `interview` sessions from `chat` sessions.
...
- **Acceptance Criteria:**
  ...
  - If no conversation thread exists yet, the response returns an empty conversation payload rather than failing.
...
Build in this order: `US-01 -> US-02 -> US-03 -> US-06 -> US-04 -> US-05 -> US-07 -> US-08 -> US-09 -> US-10 -> US-11 -> US-12 -> US-13 -> US-14`.

This gives you a dependency-safe integrated version: saved journey maps, editable matrix structure, persisted chat, safe AI-assisted updates, and frontend state that stays aligned with backend truth.