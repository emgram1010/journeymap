...
export const formatMatrixCellMarkup = ({
  ...
  // Only activate tile renderer when actorFields has at least one non-empty value;
  // otherwise fall through to plain content text (e.g. cells filled via MCP with text content only).
  const tileActorTypes = new Set(['customer', 'internal', 'engineering', 'ai_agent', 'financial', 'vendor', 'handoff']);
  const actorFieldsHasData = meta?.actorFields != null &&
    typeof meta.actorFields === 'object' &&
    Object.values(meta.actorFields as Record<string, unknown>).some(v => v != null && String(v).trim() !== '');
  if (tileActorTypes.has(actorType ?? '') && actorFieldsHasData) {
    let tile = '';
    switch (actorType) {
