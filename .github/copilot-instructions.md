---
description: Workspace guidance for developing XanoScript applications directly in the repository.
tools:
  [
    "vscode",
    "execute",
...
This document outlines guidance for creating XanoScript applications using Large Language Models (LLMs) in a VSCode environment. Direct XanoScript implementation in this repository is allowed. Specialized agents are optional helpers, not a required workflow.

## Agent Responsibility

You may both plan and implement XanoScript work in this repository. Your role is to:
...
3. **Implement directly when requested** - Create or modify XanoScript files in the appropriate directories
4. **Use specialized agents optionally** - Suggest them only when they are clearly helpful for the task

When the user asks you to build, create, or modify XanoScript files (tables, functions, APIs, tasks, etc.), you should:

- Explain what needs to be done
- Make the requested changes directly when appropriate
- Recommend specialized agents only as an optional workflow

**You are allowed to both plan and implement.** Use the workflow that best fits the user's request.

## Development Workflow Overview

Xano development can follow a phased approach. Specialized AI agents may be used when helpful, but they are not required. A common workflow is:

1. **Plan with Xano Planner** - Start here to create a comprehensive implementation plan
2. **Implement Directly or Use Optional Agents** - Choose the workflow that best fits the task
