...
query "journey_architecture/{journey_architecture_id}/compare/message" verb=POST {
  ...
  stack {
    ...
    foreach ($stages_a) {
      each as $stage {
        ...
        var.update $stage_rows_a {
          value = $stage_rows_a
            |push:{stage_key: $stage.key, stage_label: $stage.label, health: $stage_health_a, health_label: $hl_a}
        }
      }
    }
    ...
    foreach ($stages_b) {
      each as $stage {
        ...
        var.update $stage_rows_b {
          value = $stage_rows_b
            |push:{stage_key: $stage.key, stage_label: $stage.label, health: $stage_health_b, health_label: $hl_b}
        }
      }
    }
    ...
    var.update $ctx {
      value = $ctx
        |concat:" (map_id: " ~ ($input.map_a_id|to_text) ~ ")\n":""
    }
  
    var.update $ctx {
      value = $ctx
        |concat:"Journey Health: " ~ ($map_health_a|to_text) ~ " (" ~ $map_hl_a ~ ")\n":""
    }
  
    var.update $ctx {
      value = $ctx
        |concat:"Financial: RAR $" ~ ($fin_rar_a|to_text) ~ " | CTS $" ~ ($fin_cts_a|to_text) ~ " | ASY $" ~ ($fin_asy_a|to_text) ~ " | UOP $" ~ ($fin_uop_a|to_text) ~ "\n":""
    }
    ...
    foreach ($stage_rows_a) {
      each as $row {
        ...
        var.update $ctx {
          value = $ctx
            |concat:"- " ~ $row.stage_label ~ " (" ~ $row.stage_key ~ "): " ~ $rh ~ " (" ~ $rhl ~ ")\n":""
        }
      }
    }
    ...
    var.update $ctx {
      value = $ctx
        |concat:"Journey Health: " ~ ($map_health_b|to_text) ~ " (" ~ $map_hl_b ~ ")\n":""
    }
  
    var.update $ctx {
      value = $ctx
        |concat:"Financial: RAR $" ~ ($fin_rar_b|to_text) ~ " | CTS $" ~ ($fin_cts_b|to_text) ~ " | ASY $" ~ ($fin_asy_b|to_text) ~ " | UOP $" ~ ($fin_uop_b|to_text) ~ "\n":""
    }
    ...
    var.update $ctx {
      value = $ctx
        |concat:"| Stage | " ~ $map_a.title ~ " | " ~ $map_b.title ~ " | Delta | Winner |\n":""
    }
    ...
    var.update $ctx {
      value = $ctx
        |concat:"Overall: " ~ $map_b.title ~ " wins " ~ ($b_wins|to_text) ~ " stages, " ~ $map_a.title ~ " wins " ~ ($a_wins|to_text) ~ " stages\n":""
    }
  ...
  guid = "3TjJot52q8g9hOgKJblPVBFh6zI"
}