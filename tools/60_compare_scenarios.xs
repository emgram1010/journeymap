...
tool compare_scenarios {
  ...
  stack {
    ...
    var $total_a    { value = $cells_a|count }
    var $confirmed_a { value = 0 }
    var $open_a     { value = 0 }
      ...
      each as $c {
        conditional {
          if ($c.status == "confirmed") {
            var.update $confirmed_a { value = $confirmed_a + 1 }
          }
        }
        conditional {
          if ($c.status == "open") {
            var.update $open_a { value = $open_a + 1 }
          }
        }
      }
    }
          ...
          each as $sc {
            conditional {
              if ($sc.stage == $stage.id) {
                array.push $stage_cells_a { value = $sc }
              }
            }
          }
        }

        var $stage_total_a    { value = $stage_cells_a|count }
        var $stage_confirmed_a { value = 0 }

        foreach ($stage_cells_a) {
          each as $stc {
            conditional {
              if ($stc.status == "confirmed") {
                var.update $stage_confirmed_a { value = $stage_confirmed_a + 1 }
              }
            }
          }
        }
    ...
    var $total_b    { value = $cells_b|count }
    var $confirmed_b { value = 0 }

    foreach ($cells_b) {
      each as $c {
        conditional {
          if ($c.status == "confirmed") {
            var.update $confirmed_b { value = $confirmed_b + 1 }
          }
        }
      }
    }
      ...
      each as $stage {
        ...
        foreach ($cells_b) {
          each as $sc {
            conditional {
              if ($sc.stage == $stage.id) {
                array.push $stage_cells_b { value = $sc }
              }
            }
          }
        }

        var $stage_total_b    { value = $stage_cells_b|count }
        var $stage_confirmed_b { value = 0 }

        foreach ($stage_cells_b) {
          each as $stc {
            conditional {
              if ($stc.status == "confirmed") {
                var.update $stage_confirmed_b { value = $stage_confirmed_b + 1 }
              }
            }
          }
        }
